...
/**
 * Formulaire Tally incrusté en fin d'article, quand l'en-tête porte un
 * `tally_url`. On accepte l'adresse publique (tally.so/r/XXXX), celle que le
 * propriétaire a sous la main, et on en dérive l'adresse d'incrustation.
 *
 * Tally n'émet aucune hauteur — voir CLAUDE.md : ses évènements ne portent que
 * l'identifiant du formulaire. La hauteur est donc posée en dur dans le CSS,
 * à partir de mesures. Un lien de secours suit le cadre, pour le cas où
 * l'incrustation serait bloquée par le navigateur.
 */
function formulaire(a) {
  const brut = String(a.tally_url || '').trim();
  if (!brut) return '';
...
/* Sur fond sombre, le logo sans plaque : assets/logo-clair.png, dont le
   contour blanc se détache seul. La plaque de logo.png n'a servi que le
   temps du fond blanc, le 23 septembre 2026. */
...
const entete = `  <header class="blog-top wrap">
    ...
    <nav class="blog-top__liens" aria-label="Liens du site">
      <a class="blog-top__guide mono" href="guide.html">Guide gratuit</a>
